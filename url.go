// Package url (inspired from the standard "net/url" package) parses URLs and implements query escaping
package url
...
var (
	urlPool sync.Pool = sync.Pool{
		New: func() interface{} {
			return new(url)
		},
	}
	userInfoPool sync.Pool = sync.Pool{
		New: func() interface{} {
			return new(userInfo)
		},
	}
	stringsBuilderPool sync.Pool = sync.Pool{
		New: func() any {
...
func stringsBuilderResetPut(sb *strings.Builder) {
	if sb != nil {
		sb.Reset()
...
// New instance from the url structure pool
func New() *url {
	u := urlPool.Get().(*url)
	return u
}
...
func unescape(s string, mode encoding) (string, error) {
	...
	t := stringsBuilderPool.Get().(*strings.Builder)
	t.Grow(len(s) - 2*n)
	for i := 0; i < len(s); i++ {
		switch s[i] {
		case '%':
...
func escape(s string, mode encoding) string {
	spaceCount, hexCount, sLen := 0, 0, len(s)
	for i := 0; i < len(s); i++ {
		c := s[i]
	...
	if hexCount == 0 {
		...
		return nocopy.ByteSliceToString(t)
	}
...
// ActualRedacted is like String but replaces all password characters with "x".
// Only the password in u.User is redacted.
func (u *url) ActualRedacted() string {
	if u == nil {
		return ""
	}

	ru := *u
	if pass, has := ru.User.Password(); has {
		redacted, passLen := "", len(pass)
		...
		redacted = sb.String()
		stringsBuilderResetPut(sb)
		ru.User = UserPassword(ru.User.Username(), redacted)
	}
	return ru.String()
}
...
func (v Values) Set(key, value string) {
	v[key] = nocopy.StringToStringSlice(value)
}
...
func resolvePath(base, ref string) string {
	...
	elem, dst, first, remaining := "", stringsBuilderPool.Get().(*strings.Builder), true, full
...
func (u *url) MarshalBinary() (text []byte, err error) {
	return nocopy.StringToByteSlice(u.String()), nil
}

func (u *url) UnmarshalBinary(text []byte) error {
	u1, err := Parse(nocopy.ByteSliceToString(text))
	if err != nil {
		return err
	}
	*u = *u1
...
func (u *url) JoinPath(elem ...string) *url {
	elem = append(nocopy.StringToStringSlice(u.EscapedPath()), elem...)
	var p string
...
/*
Resets the "userInfo" struct and if put is true
then it puts the resetted "userInfo" struct into the pool.

Note: be careful of the put parameter, just set it true
when you don't want to reuse the "userInfo" struct anymore.
*/
func (u *userInfo) reset(put bool) {
	if u != nil {
		u.password = ""
		u.username = ""
		u.passwordSet = false
...
/*
Resets both "url" and "userInfo" structs and if put is true
then it puts both resetted "url" and "userInfo" structs into the pool.

NOTE: be careful of the put parameter, just set it true
when you don't want to reuse both "url" and "userInfo" structs anymore.
*/
func (u *url) Reset(put bool) {
	u.User.reset(put)
	u.User = nil
